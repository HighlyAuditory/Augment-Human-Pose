...
class TransferModel(BaseModel):
    ...
    def forward(self, input_BP_res):
        ...
        fake_p2_padded = heatmap_pose.preprocess(self.fake_p2, [256, 176])
        heatmap = heatmap_pose.process(self.cpm_model, fake_p2_padded, self.mask)
        self.heat6 = heatmap
        self.input_BP_res = input_BP_res

        return self.heat6, self.input_BP_res
    ...
    def backward_G_double(self, heatmaps, BPs):
        # no gan loss for now
        t = Variable(BPs, requires_grad=False)
        pl = self.pose_loss(torch.clamp(heatmaps, min=0, max=1), t)*self.opt.lambda_pose / 2
        self.acc_pl += pl
        pl.backward()
        return 
