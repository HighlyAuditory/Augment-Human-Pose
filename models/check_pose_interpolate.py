...
def demo_image(image, model, opt, name):
  ...
  dic = {'2d': pred_2d,'absolute_angles': absolute_angles, 'limbs':limbs, 'offset': offset}
  
  return absolute_angles, limbs, offset, pred_2d
  # rev_3d = anglelimbtoxyz2(offset, absolute_angles, limbs)
  # print(name)
  # debugger = Debugger()
...
def interpolate(abs_angles1, abs_angles2, factor=0):
  return factor * abs_angles1 + (1-factor) * abs_angles2
...
def main(opt):
  ...
  if os.path.isdir(opt.demo):
    ...
    def demo_single(opt, file_name):
        ...
        return demo_image(image, model, opt, name.replace('.jpg', '.npy'))

    absolute_angles1, _, _, _ = demo_single(opt, file_name1)
    absolute_angles2, limbs, offset, pred_2d = demo_single(opt, file_name2)

    aug_angles = interpolate(absolute_angles1, absolute_angles2)
    ...
    aug_pose = check_visibility(aug_angles, aug_pose)
